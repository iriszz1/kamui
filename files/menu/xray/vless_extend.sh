...
exp_old=$(cat /kamui/user_database.json | jq -r '(.xray.vless[] | select(.username == "'$username'")).expired')
diff=$((($(date -d "${exp_old}" +%s)-$(date +%s))/(86400)))
duration=$(expr $diff + $days + 1)
exp_new=$(date -d +${duration}days +%Y-%m-%d)

cat /kamui/user_database.json | jq '(.xray.vless[] | select(.username == "'$username'")).expired |= "'$exp_new'"' > /kamui/user_database.json.tmp
